...
def buyandhold(
    ...
    # DataFrame을 복사 copy()
    result = _df.copy()
    # 인덱스가 0부터 시작하는 인덱스 // Date가 인덱스인 경우 
    ...
    try : 
        ...
        # _end는 기본값이 시계열인 경우는 아무 행동도 하지 않는다.
        ...
        print(f"시작시간과 종료시간의 포맷은 yyyy-mm-dd 형식입니다")
        return ""
    ...
    print(f"""{start.strftime('%Y-%m-%d')}부터
          {start.strftime('%Y-%m-%d')}까지
          buyandhold의 수익률은 {result.iloc[-1, 2]}입니다""")
